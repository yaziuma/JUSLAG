...
3. 2026-09-17: バックテストの損益と寄りgapは、シグナル日より後の最初のJP価格行へ対応付けるよう修正。gap欠損時に翌日の値を代用しない。JP休場日とgap欠損のテストを追加。日次シグナル生成には、共通日の後に米国だけ開場した最新リターンを取り込むオプションを追加（履歴バックテストは従来のまま）。ただしJP休日でJP最新日がUSシグナル日より古い場合、現行の鮮度ゲートはなお見送りにする。休日を考慮した鮮度検証、取引時刻に基づく利用可能性、価格リターンの共通日化順序の統一は未完了。
