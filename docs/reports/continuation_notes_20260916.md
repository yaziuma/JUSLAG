...
3. 2026-09-17: バックテストの損益と寄りgapは、シグナル日より後の最初のJP価格行へ対応付けるよう修正。gap欠損時に翌日の値を代用しない。JP休場日とgap欠損のテストを追加。ただし、シグナル生成はなお日米共通日だけを使う。米国のみ開場した日の最新情報を日次計算に取り込むこと、取引時刻に基づく利用可能性の検証、価格リターンの共通日化順序の統一は未完了。
