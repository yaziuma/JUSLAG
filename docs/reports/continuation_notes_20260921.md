...
- SBIの公開ルール、日次売建条件、料金は調査可能であり、保存項目を[`sbi_execution_evidence_plan_20260921.md`](sbi_execution_evidence_plan_20260921.md)に固定した。個別注文の受付・約定証拠だけは実注文後の口座履歴を必要とする。
- SBI公開差分コレクターと平日08:45/19:10 JSTのローカルtimer、GitHub補助workflowを実装した。公開全一覧はログイン必須なので、差分非掲載を売建不可とは判定しない。A2ゲート出力へ情報専用の収集状況を追加した。
