...
約1年の direction flip 検証で「全体は現行方向が優位、ただし 2026年4月は反転優位」という
局面依存パターンが観察された。本検証では各日に局面ラベルを付与し、
**「どんな局面で現行方向 / 反転方向が有利か」** を切り分ける。
...
### 7-3. 2026年4月の反転優位について

- 4月異常日の局面構成: trend: {'downtrend': 2, 'sideways': 1}, vol: {'high_vol': 3}
- 4月に反転優位が急拡大した背景として以下が考えられる:
  - **downtrend × high_vol** 局面への突入（日米関税ショック等）
  - セクター間ばらつき（rotation 強度）の急変
...
「現行方向が常に正しい」とは言えず、局面によって方向判断を変える合理性がある。
